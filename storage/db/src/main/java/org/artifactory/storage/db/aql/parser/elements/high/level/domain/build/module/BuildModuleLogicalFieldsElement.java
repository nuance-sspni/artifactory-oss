...
import static org.artifactory.storage.db.aql.parser.AqlParser.buildArtifactLogicalFields;
import static org.artifactory.storage.db.aql.parser.AqlParser.buildDependenciesLogicalFields;
import static org.artifactory.storage.db.aql.parser.AqlParser.buildLogicalFields;
import static org.artifactory.storage.db.aql.parser.AqlParser.buildModulePropertiesLogicalFields;
 ...
 */
public class BuildModuleLogicalFieldsElement extends LazyParserElement implements DomainProviderElement {
    @Override
    ...
    private void fillWithDomainFields(List<ParserElement> list) {
        AqlLogicalFieldEnum[] fields = modules.getLogicalFields();
        for (AqlLogicalFieldEnum field1 : fields) {
            list.add(new RealFieldElement(field1.getSignature(), modules));
        }
    }

    private void fillWithSubDomains(List<ParserElement> list) {
        list.add(forward(new InternalNameElement(builds.signature), dot, buildLogicalFields));
        ...
        list.add(forward(new InternalNameElement(moduleProperties.signature), dot, buildModulePropertiesLogicalFields));
    }
