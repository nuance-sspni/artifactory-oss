 ...
 */
@Service
public class UserGroupStoreServiceImpl implements UserGroupStoreService {
    ...
    public UserInfo findUser(String username, List<String> propertiesToRetrieve) {
        try {
            ...
            //retrive user with wanted properties
            if (propertiesToRetrieve == null) {
                ...
                Set<UserPropertyInfo> requiredProperties = AllUserProperties.stream()
                        .filter(userPropertyInfo -> propertiesToRetrieve.contains(userPropertyInfo.getPropKey()))
                        .collect(Collectors.toSet());
                return userToUserInfoWithProperties(user, requiredProperties);
            }
        } catch (SQLException e) {
    ...
    public List<GroupInfo> getAllGroups(boolean includeAdmins) {
        return findAllGroups(includeAdmins ? UserGroupsDao.GroupFilter.ALL : UserGroupsDao.GroupFilter.EXCLUDE_ADMINS);
    }
     ...
     * attempt,
     * <p>
     * - delay becomes affective after {@link UserGroupStoreServiceImpl#OK_INCORRECT_LOGINS}
     * - maximum possible delay is {@link UserGroupStoreServiceImpl#MAX_LOGIN_DELAY}
     *
     * @return login delay or -1 if user login should
    ...
    public long getNextLogin(int incorrectLoginAttempts, long lastAccessTimeMillis) {
        if (incorrectLoginAttempts >= OK_INCORRECT_LOGINS) {
            long delay = (long) ((incorrectLoginAttempts - OK_INCORRECT_LOGINS) * LOGIN_DELAY_MULTIPLIER);
            if (delay != 0) {
    ...
    public Map<String, Long> getAllGroupIdsToNames() {
        try {
            return userGroupsDao.getAllGroupIdsToNames();
        } catch (SQLException e) {
            throw new StorageException("Could not get all admin groups, cause: {}", e);
        }
    }
    ...
    public Multimap<Long, Long> getAllUsersInGroups() {
        try {
            return userGroupsDao.getAllUsersInGroups();
        } catch (SQLException e) {
            throw new StorageException("Could not get all group users, cause: {}", e);
        }
    }
