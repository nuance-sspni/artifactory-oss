...
public class BuildValuesElement extends LazyParserElement implements DomainProviderElement {
    ...
    private void fillWithSubDomains(List<ParserElement> list) {
        list.add(forward(new InternalNameElement(modules.signature), dot, buildModuleValues));
        list.add(forward(new InternalNameElement(buildPromotions.signature), dot, buildPromotionsValues));
