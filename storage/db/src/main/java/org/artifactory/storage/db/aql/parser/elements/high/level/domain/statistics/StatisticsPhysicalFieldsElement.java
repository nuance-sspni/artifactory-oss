...
package org.artifactory.storage.db.aql.parser.elements.high.level.domain.statistics;
 ...
 */
public class StatisticsPhysicalFieldsElement extends LazyParserElement implements DomainProviderElement {
    @Override
    ...
    private void fillWithDomainFields(List<ParserElement> list) {
        AqlPhysicalFieldEnum[] fields = statistics.getPhysicalFields();
        for (AqlPhysicalFieldEnum field : fields) {
            list.add(new RealFieldElement(field.getSignature(), statistics));
        }
    }

    private void fillWithSubDomains(List<ParserElement> list) {
        list.add(forward(forward(new InternalNameElement(items.signature), dot, itemPhysicalFields)));
    }
