...
public class VirtualRepoCriteriaDecorator implements DecorationStrategy {
    ...
    public void decorate(AqlQuery aqlQuery, AqlQueryDecoratorContext decoratorContext) {
        ...
        int index = 0;
        while ((index = findNextRepoCriteria(index, aqlQueryElements)) >= 0) {
            index += handleRepoCriteria(index, aqlQueryElements, decoratorContext);
        }
    }

    private int findNextRepoCriteria(int start, List<AqlQueryElement> aqlQueryElements) {
        int index = start;
        for (; index < aqlQueryElements.size(); index++) {
            ...
            if (element instanceof Criterion) {
                ...
                if (var1 instanceof AqlField && ((AqlField)var1).getFieldEnum() == AqlPhysicalFieldEnum.itemRepo) {
                    return index;
                }
            }
        }
