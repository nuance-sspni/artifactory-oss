...
public class NodesDao extends BaseDao {
    ...
    public List<Node> searchNodesByProperty(String repo, String propKey) throws SQLException {
        List<Node> results = new ArrayList<>();
        String query = "SELECT n.* FROM nodes n JOIN node_props p " +
                "ON n.node_id = p.node_id " +
                "WHERE repo = ? AND p.prop_key = ?";
        try (ResultSet resultSet = jdbcHelper.executeSelect(query, repo, propKey)){
            while (resultSet.next()) {
                results.add(nodeFromResultSet(resultSet));
            }
        }
    ...
    private Node nodeFromResultSet(ResultSet resultSet) throws SQLException {
        ...
        return new Node(nodeId, isFile, repoName, path, fileName, depth, created, createdBy,
