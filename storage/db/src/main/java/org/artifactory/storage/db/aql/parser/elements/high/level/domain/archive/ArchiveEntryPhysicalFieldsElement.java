...
import static org.artifactory.storage.db.aql.parser.AqlParser.archivePhysicalFields;
import static org.artifactory.storage.db.aql.parser.AqlParser.dot;

/**
 * @author Gidi Shabat
 */
public class ArchiveEntryPhysicalFieldsElement extends LazyParserElement implements DomainProviderElement {
    ...
    private void fillWithDomainFields(List<ParserElement> list) {
        AqlPhysicalFieldEnum[] fields = entries.getPhysicalFields();
        for (AqlPhysicalFieldEnum field : fields) {
            list.add(new RealFieldElement(field.getSignature(), entries));
        }
    }
