...
public class ArchiveValuesElement extends LazyParserElement implements DomainProviderElement {
    ...
    private void fillWithSubDomains(List<ParserElement> list) {
        list.add(forward(forward(new InternalNameElement(items.signature), forward(dot, itemValues))));
        list.add(forward(forward(new InternalNameElement(entries.signature), forward(dot, entriesValues))));
    }
