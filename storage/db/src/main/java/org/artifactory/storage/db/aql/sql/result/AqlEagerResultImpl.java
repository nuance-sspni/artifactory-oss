...
public class AqlEagerResultImpl<T extends AqlRowResult> implements AqlEagerResult {
    ...
    public AqlEagerResultImpl(ResultSet resultSet, SqlQuery sqlQuery, AqlRepoProvider repoProvider) throws SQLException {
    ...
    public T getResult(int i) {
        return (T) new AqlBaseFullRowImpl(toMapByFieldEnum(rows.get(i).toMap()));
    }
    ...
    public List<T> getResults() {
        ...
        for (EagerRowResult row : rows) {
            result.add(new AqlBaseFullRowImpl(toMapByFieldEnum(row.toMap())));
        }
        return result;
    }
