...
public abstract class UpgradeBaseTest extends AbstractTestNGSpringContextTests {
    ...
    protected void rollBackTo300Version() throws SQLException, IOException {
        try (Connection connection = jdbcHelper.getDataSource().getConnection()) {
            ...
            CommonDbUtils.executeSqlStream(connection, getPreviousDbSchemaSql("v300", dbProperties.getDbType()));
            CommonDbUtils.executeSqlStream(connection, getPreviousImportSql("v300", "user-group"));
            CommonDbUtils.executeSqlStream(connection, getPreviousImportSql("v300", "acls"));
            CommonDbUtils.executeSqlStream(connection, getPreviousImportSql("v300", "builds"));
            CommonDbUtils.executeSqlStream(connection, getPreviousImportSql("v300", "nodes"));
        }
    }
