...
public class StatsServiceImplTest extends DbBaseTest {
    ...
    @Test
    public void getStatsFileInfoWithStats() {
        StatsInfo stats = statsService.getStats(
    ...
    @Test
    public void getStatsFileInfoWithoutIdWithStats() {
        StatsInfo stats = statsService.getStats(new FileInfoImpl(new RepoPathImpl("repo1", "ant/ant/1.5/ant-1.5.jar")));
        assertNotNull(stats);
        assertEquals(stats.getDownloadCount(), 2);
        assertEquals(stats.getRemoteDownloadCount(), 3);
    }

    public void getStatsFileInfoWithNoStats() {
        assertNull(statsService.getStats(new FileInfoImpl(new RepoPathImpl("repo2", "org/jfrog/test/test.jar"), 504)));
    }
    ...
    public void fileDownloadedOnNonExistingItem() {
        ...
        StatsInfo statsFromDb = ReflectionTestUtils.invokeMethod(
                statsService, "getStatsFromStorage", filePath, (long) DbService.NO_DB_ID);
    ...
    public void setRemoteStatsOnFileWithStats() {
        RepoPathImpl filePath = new RepoPathImpl("repo2", "org/jfrog/test/test2.pom");
        assertNotNull(statsService.getStats(filePath));

        long lastDownloaded = System.currentTimeMillis() + 3000;
        StatsImpl statsInfo = new StatsImpl();
        statsInfo.setDownloadCount(1234);
        statsInfo.setLastDownloaded(lastDownloaded);
        statsInfo.setLastDownloadedBy("blabla");
        statsInfo.setRemoteDownloadCount(2345);
        statsInfo.setRemoteLastDownloaded(lastDownloaded+1);
        statsInfo.setRemoteLastDownloadedBy("foofoo");
        statsInfo.setOrigin("the-origin");
        statsInfo.setPath("the->path");
        statsService.setStats(505, statsInfo);

        StatsInfo stats = statsService.getStats(filePath);
        assertNotNull(stats);
        assertEquals(stats.getDownloadCount(), 1234);
        assertEquals(stats.getLastDownloaded(), lastDownloaded);
        assertEquals(stats.getLastDownloadedBy(), "blabla");
        assertEquals(stats.getRemoteDownloadCount(), 2345);
        assertEquals(stats.getRemoteLastDownloaded(), lastDownloaded+1);
        assertEquals(stats.getRemoteLastDownloadedBy(), "foofoo");
        assertEquals(stats.getOrigin(), "the-origin");
        assertEquals(stats.getPath(), "the->path");
    }

    @Test(dependsOnMethods = "setRemoteStatsOnFileWithStats")
