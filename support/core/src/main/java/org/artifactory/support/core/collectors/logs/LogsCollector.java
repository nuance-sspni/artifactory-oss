...
public class LogsCollector extends AbstractSpecificContentCollector<SystemLogsConfiguration> {
    ...
    protected boolean doCollect(SystemLogsConfiguration configuration, File tmpDir) {
        try {
            collectArtifactoryLogs(configuration, tmpDir);
            collectAccessLogs(tmpDir);
    ...
    private void collectArtifactoryLogs(SystemLogsConfiguration configuration, File tmpDir) throws IOException {
    ...
    private void collectAccessLogs(File tmpDir) throws IOException {
        Files.walk(getAccessLogsDirectory().toPath(), FileVisitOption.FOLLOW_LINKS)
                .filter(Files::isRegularFile)
                .forEach(f -> {
                    Path relativePath = f.getParent().relativize(getAccessLogsDirectory().toPath());
                    File targetLocation = tmpDir.toPath().resolve(relativePath).toFile();
                    try {
                        FileUtils.forceMkdir(targetLocation);
                    } catch (IOException e) {
                        getLog().error("Failed to create a directory", e);
    ...
    /**
     * @return Access server logs directory under the artifactory home
     */
    // Access logs path is hardcoded since there is no exposure of the path from AccessService.
    // Logs will move out from Artifactory once JFrog Platform is out
    private File getAccessLogsDirectory() {
        return new File(ArtifactoryHome.get().getHomeDir(),"access/logs");
    }
