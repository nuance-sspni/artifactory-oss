...
public class RemoteRepoBaseTest extends ArtifactoryHomeBoundTest {
    ...
    XrayAddon xrayAddon = EasyMock.createMock(XrayAddon.class);
    ...
    public void setup() {
        ...
        EasyMock.expect(addonsManager.addonByType(XrayAddon.class)).andReturn(xrayAddon).anyTimes();
    ...
    private void createCacheRepo() {
        ...
        DbStoringRepoMixin mixin = EasyMock.createMock(DbStoringRepoMixin.class);
        ReflectionTestUtils.setField(localCacheRepo, "mixin", mixin);
        EasyMock.expect(mixin.itemExists(anyObject())).andReturn(true).anyTimes();
        EasyMock.expect(mixin.getKey()).andReturn("remote-repo-cache").anyTimes();
        replay(mixin);
    }
    ...
    private void expectXrayBlocked(boolean blocked) {
        EasyMock.expect(xrayAddon.isDownloadBlocked(anyObject(RepoPath.class))).andReturn(blocked).anyTimes();
        EasyMock.replay(xrayAddon);
    }
    ...
    private void resetMocks() {
        EasyMock.reset(authService, xrayAddon);
    }
}
