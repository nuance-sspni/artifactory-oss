...
public class ArtifactoryRequestTest extends ArtifactoryHomeBoundTest {
    ...
    public void removeDotSegment() throws UnsupportedEncodingException {
        ArtifactoryRequestBase request = newRequest();
        request.setRepoPath(request.calculateRepoPath("libs-releases/path1/path2/./file.jar"));
        RepoPath repoPath = request.getRepoPath();
        assertEquals(repoPath.getPath(), "path1/path2/file.jar");
        assertEquals(repoPath.getRepoKey(), "libs-releases");
        request = newRequest();
        request.setRepoPath(request.calculateRepoPath("libs-releases/path1/path2/.file.jar"));
    ...
    public void matrixParamsWithDotInPath() throws UnsupportedEncodingException {
        ArtifactoryRequestBase request = newRequest();
        // expect the /./ to be replaced with /
        request.setRepoPath(request.calculateRepoPath("libs-releases/path1/./path2;a=1;a=11;b=2;c;d=4;e"));
        assertEquals(request.getPath(), "path1/path2");
        Properties params = request.getProperties();
        assertMatrixParams(params);
        request = newRequest();
        // expect the /./ to be replaced with /
        request.setRepoPath(request.calculateRepoPath("libs-releases;a=1;a=11;b=2;c;d=4;e/path1/./path2"));
        assertEquals(request.getPath(), "path1/path2");
        params = request.getProperties();
        assertMatrixParams(params);
        request = newRequest();
        // expect the repopath to be a file with props
        request.setRepoPath(request.calculateRepoPath("libs-releases;a=1;b=2/path1/path2;a=11;c;d=4;e"));
        assertEquals(request.getPath(), "path1/path2");
        assertTrue(request.getRepoPath().isFile());
        params = request.getProperties();
        assertMatrixParams(params);
        request = newRequest();
        // Expect the /. to be replaced with /
        request.setRepoPath(request.calculateRepoPath("libs-releases;a=1;a=11;b=2;c;d=4;e/folder/folder2/."));
        assertEquals(request.getPath(), "folder/folder2");
        assertTrue(request.getRepoPath().isFolder());
        params = request.getProperties();
        assertMatrixParams(params);
    }
    ...
    public void zipResourceAbsolute2() throws UnsupportedEncodingException {
        ArtifactoryRequestBase request = newRequest();
        request.setRepoPath(request.calculateRepoPath("libs-releases/path/to/./zip!/zip/resource"));
        assertEquals(request.getRepoPath(), InfoFactoryHolder.get().createRepoPath("libs-releases", "path/to/zip"));
        assertEquals(request.getZipResourcePath(), "zip/resource");
