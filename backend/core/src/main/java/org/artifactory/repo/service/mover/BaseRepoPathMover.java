...
public abstract class BaseRepoPathMover {
    ...
    protected boolean canMove(VfsItem source, RepoRepoPath<LocalRepo> targetRrp) {
        ...
        if (!canMoveByRepoType(source, targetRrp)) {
            return false;
        }
    ...
    private boolean canMoveByRepoType(VfsItem source, RepoRepoPath<LocalRepo> targetRrp) {
        if (!canMoveByPom(source, targetRrp)) {
            return false;
        } else if (!canMoveByConan(source, targetRrp)) {
            return false;
        }
        return true;
    }

    private boolean canMoveByPom(VfsItem source, RepoRepoPath<LocalRepo> targetRrp) {
        RepoPath sourceRepoPath = source.getRepoPath();
        String targetPath = targetRrp.getRepoPath().getPath();
        LocalRepo targetRepo = targetRrp.getRepo();
    ...
    private boolean canMoveByConan(VfsItem source, RepoRepoPath<LocalRepo> targetRrp) {
        RepoType targetRepoType = targetRrp.getRepo().getDescriptor().getType();
        if (RepoType.Conan.equals(targetRepoType)) {
            String sourcePath = source.getRepoPath().getPath();
            String targetPath = targetRrp.getRepoPath().getPath();
            if (!targetPath.equals(sourcePath)) {
                status.error("Target path for Conan files must match the source path: " + targetPath,
