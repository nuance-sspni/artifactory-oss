...
public class RepositoryServiceImpl implements InternalRepositoryService {
    ...
    public MoveMultiStatusHolder move(Set<RepoPath> pathsToMove, String targetLocalRepoKey,
        ...
        for (RepoPath pathToMove : pathsToMoveIncludingParents) {
            RepoPath targetRepoPath = InternalRepoPathFactory.create(targetLocalRepoKey, pathToMove.getPath(), pathToMove.isFolder());
    ...
    public MoveMultiStatusHolder copy(Set<RepoPath> pathsToCopy, String targetLocalRepoKey,
        ...
        for (RepoPath pathToCopy : pathsToCopyIncludingParents) {
            RepoPath targetRepoPath = InternalRepoPathFactory.create(targetLocalRepoKey, pathToCopy.getPath(), pathToCopy.isFolder());
    ...
    @Override
    public BasicStatusHolder undeployInTx(RepoPath repoPath, boolean calcMavenMetadata, BasicStatusHolder statusHolder) {
    ...
    public StatusHolder undeployVersionUnits(Set<VersionUnit> versionUnits) {
        ...
        if( ! ConstantValues.mvnMetadataCalculationSkipDeleteEvent.getBoolean()) {
            pathsForMavenMetadataCalculation.stream()
                    .filter(this::exists)
                    .forEach(path -> mavenMetadataService.calculateMavenMetadataAsync(new MavenMetadataWorkItem(path, false)));
        }
        return status;
    }
    ...
    public List<LocalCacheRepoDescriptor> getCachedRepoDescriptors() {
        return cache.get().localCacheRepositoriesMap.values().stream().map(Repo::getDescriptor)
                .collect(Collectors.toList());
    }

    @Override
    public List<DistributionRepoDescriptor> getDistributionRepoDescriptors() {
        return cache.get().distributionRepositoriesMap.values().stream().map(Repo::getDescriptor)
                .collect(Collectors.toList());
    }
    ...
    public boolean isRepoPathAccepted(RepoPath repoPath) {
        ...
        if (repo instanceof RepoBase) {
            return ((RepoBase)repo).accepts(repoPath);
        }
        return true;
    }
    ...
    private List<String> getLocalAndCacheRepoKeys() {
        ...
        for (String key : cache.get().distributionRepositoriesMap.keySet()) {
            result.add(key);
        }
        return result;
    }
    ...
    private CloseableHttpResponse executeGetMethod(String url, Map<String, String> headersMap) throws IOException {
        ...
        CloseableHttpClient client = configurator.proxy(proxy).build();
    ...
    private class RepositoriesCacheBuilder {
        ...
        public RepositoriesCache rebuildRepositories() {
        ...
        private Map<String, LocalRepoDescriptor> rebuildLocalRepositories(CentralConfigDescriptor centralConfig,
        ...
        private Map<String, RemoteRepoDescriptor> rebuildRemoteAndCacheRepositories(
                CentralConfigDescriptor centralConfig, InternalRepositoryService transactionalMe) {
        ...
        private void rebuildVirtualRepositories(CentralConfigDescriptor centralConfig,
                InternalRepositoryService transactionalMe, Map<String, LocalRepoDescriptor> localRepoDescriptorMap,
                Map<String, RemoteRepoDescriptor> remoteRepoDescriptorMap) {
        ...
        private void rebuildDistributionRepositories(CentralConfigDescriptor centralConfig,
                InternalRepositoryService transactionalMe) {
            Map<String, DistributionRepoDescriptor> distRepoDescriptorMap = centralConfig
                    .getDistributionRepositoriesMap();
            ...
            for (DistributionRepoDescriptor repoDescriptor : distRepoDescriptorMap.values()) {
                ...
                if (!distributionRepositoriesMap.isEmpty()) {
                    ...
                    if (oldRepo != null) {
                        ...
                        // This could be a new repo that is in the newly saved config but not in the global map yet.
                        // Only if we do not find it there as well then it is an error
                        DistributionRepoDescriptor newDistRepo = centralConfig.getDistributionRepositoriesMap()
                                .get(key);
                        if (newDistRepo == null) {
                            log.error("Reloading configuration did not find local repository " + key);
                        }
    ...
    private class RepositoriesCacheLoader implements Callable<RepositoriesCache> {
        ...
        public RepositoriesCache call() throws Exception {
            InternalRepositoryService transactionalMe = getTransactionalMe();
            RepositoriesCache cache = transactionalMe.rebuildRepositoriesInTransaction();
            return cache;
        }
    }
}
