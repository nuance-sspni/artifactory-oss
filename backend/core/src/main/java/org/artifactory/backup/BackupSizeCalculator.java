...
public class BackupSizeCalculator {
    ...
    public BackupSizeCalculator(RepositoryService repositoryService,
            BuildStoreService buildStoreService, ConfigsService configsService, BackupService backupService,
            File dataDir, File etcDir) {
    ...
    boolean isEnoughFreeSpace(@Nonnull ExportSettingsImpl settings) {
        ...
        try {
            ...
            log.error("Can't estimate free space in backup target location path. starting backup without space check");
            return true;
        }
        long backupSize = 0;
        ...
        if (configsService.getConfig("licenses.xml") != null) {
            backupSize += roundSizeUpTo(configsService.getConfig("licenses.xml").length(), ROUND_UP_SIZE);
        }
    ...
    private long getBackupFilesSize() {
        ...
        File artifactoryProperties = findFile(dataDir.listFiles(), "artifactory.properties");
        ...
        return configFiles.stream().mapToLong(file -> roundSizeUpTo(file.length(), ROUND_UP_SIZE)).sum();
    }
    ...
    private long getBuildsSize() {
        return buildStoreService.getAllBuildNames().stream()
                ...
                .map(buildStoreService::getExportableBuild)
                .mapToLong(exportedBuild -> roundSizeUpTo(exportedBuild.getJson().length(), ROUND_UP_SIZE))
                .sum();
    }
     ...
     */
    private long roundSizeUpTo(long x, int multiple) {
