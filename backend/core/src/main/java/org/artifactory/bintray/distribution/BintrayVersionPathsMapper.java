...
class BintrayVersionPathsMapper {
    ...
    BintrayVersionPathsMapper(DistributionRepoDescriptor targetRepoDescriptor, DistributionReporter status) {
    ...
    Multimap<BintrayUploadInfo, DistributionCoordinatesResolver> mapPaths(List<String> requestedPaths) {
        List<RepoPath> paths = ContextHelper.get().beanForType(DistributionService.class)
                .inputPathsToRepoPaths(requestedPaths, status);
    ...
    private Multimap<BintrayUploadInfo, DistributionCoordinatesResolver> coordinatesForPaths(List<RepoPath> paths) {
        ...
        List<DistributionCoordinatesResolver> resolvers =
                getCoordinatesForPaths(pathInfo, targetRepoDescriptor.getRules(), productName, status).stream()
    ...
    private void populateRepoKeyToTypeMap(Map<String, RepoType> repoTypes, String repoKey) {
        if (repoTypes.get(repoKey) == null) {
            RepoBaseDescriptor descriptor = ContextHelper.get().beanForType(RepositoryService.class)
    ...
    private RepoLayout getRepoLayout(String repoKey) {
        RepoDescriptor descriptor = ContextHelper.get().beanForType(RepositoryService.class).repoDescriptorByKey(repoKey);
