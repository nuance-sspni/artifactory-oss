...
public class DistributorImpl implements Distributor {
    ...
    public DistributionReporter distributeInternal(Distribution distribution) {
        ...
        BintrayVersionPathsMapper mapper = new BintrayVersionPathsMapper(descriptor, status);
        Multimap<BintrayUploadInfo, DistributionCoordinatesResolver> filesPerVersion =
                mapper.mapPaths(distribution.getPackagesRepoPaths());
        ...
        filesPerVersion.keySet().stream()
                ...
                .forEach(version -> distributeVersion(distribution, descriptor, filesPerVersion, bintraySubject,
                        version, status));
        return status;
    }
    ...
    private void distributeVersion(Distribution distribution, DistributionRepoDescriptor descriptor,
        ...
        try {
            ...
            VersionHandle bintrayVersion = fetchBintrayVersion(bintraySubject, distribution, version,
                    bintrayRepoModel, status);
    ...
    public void signAndPublish(Distribution distribution, VersionHandle bintrayVersion, int versionFilesSize,
        ...
        if (canSign || canPublish) {
            ...
            if (canPublish) {
                //TODO [by dan]: add polling on artifact that was signed..? --> we publish stuff before signing was done in some cases
    ...
    private VersionHandle getDockerVersion(SubjectHandle subject, BintrayUploadInfo uploadInfo, boolean isDryRun,
        ...
        RepositoryHandle btRepo = getOrCreateRepo(uploadInfo.getRepositoryDetails(), subject, isDryRun,
                bintrayRepoModel, status);
    ...
    private VersionHandle getOrCreateVersion(VersionDetails versionDetails, PackageHandle packageHandle,
        ...
        try {
            ...
            handleException(null, "Error retrieving information for version" + versionDetails.getRepo() + "/" +
                    versionDetails.getPkg() + "/" + versionDetails.getName(), e, status, log);
            throw e;
        }
    ...
    private boolean verifyCanDistribute(@Nullable DistributionRepo repo, String distributionRepoKey,
        ...
        if (repo == null) {
            ...
            status.error("User " + authService.currentUsername() + " Is missing the required permission to distribute" +
                    " to repo " + distributionRepoKey + ", aborting.", HttpStatus.SC_FORBIDDEN, log);
            canDistribute = false;
    ...
    private boolean canPublish(Distribution distribution, RepoType type) {
        ...
        switch (type) {
            case Docker:
                canPublishType = false;
                break;
            default:
                canPublishType = true;
        }
        return canPublishType
                && (distribution.isPublish() == null || (distribution.isPublish() != null && distribution.isPublish()));
    }
