...
class BintrayVersionDistributor {
    ...
    private void distributeDocker(DistributionCoordinatesResolver coordinates) throws ItemNotFoundRuntimeException, BintrayCallException {
    ...
    RepoPath getDockerImageDistPath(DistributionCoordinatesResolver coordinates) {
        ...
        String dockerRepo = DistributionUtils.getTokenValueByPropKey(coordinates, "docker.repoName");
        String dockerManifest = DistributionUtils.getTokenValueByPropKey(coordinates, "docker.manifest");
        return RepoPathFactory.create(repoDescriptor.getKey(),
