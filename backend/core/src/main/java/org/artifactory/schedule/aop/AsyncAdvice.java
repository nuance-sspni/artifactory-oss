...
public class AsyncAdvice implements MethodInterceptor {
        ...
        try {
            if (delayExecutionUntilCommit && inSession) {
                ...
                if (traceableInvocation.getWorkExecution() != null) {
                    ...
                    future = submit(traceableInvocation.getWorkExecution());
                } else {
    ...
    private Future<?> submit(final List<WorkExecution> workExecutions) {
        workExecutions.forEach(this::submit);
        return null;
    }

    private Future<?> submit(WorkExecution workExecution) {
        WorkQueue<WorkItem> workQueue = workExecution.workQueue;
        Future<?> result = submitWorkQueueTask(workQueue, workExecution.workItem);
    ...
    private Future<?> submitWorkQueueTask(WorkQueue<WorkItem> workQueue, WorkItem workItem) {
        Future<?> result = null;
        boolean added = workQueue.offerWork(workItem);
        if (!added) {
    ...
    public static class MethodCallbackSessionResource implements SessionResource {
        ...
        final List<WorkExecution> sharedWorkQueueExecutions = new ArrayList<>();
        AsyncAdvice advice;
        ...
        public void addInvocation(TraceableMethodInvocation invocation) {
            if (invocation.getWorkExecution() != null) {
                sharedWorkQueueExecutions.add(invocation.getWorkExecution());
            } else {
