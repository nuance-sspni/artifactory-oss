...
public class PasswordExpireNotificationJob extends QuartzCommand {
    ...
    private String getProfilePageUrl() {
        ...
        String profilePageUrl = addon.getMailConfigArtifactoryUrl();
        // In case we have no url at all, just return relative url, better than nothing.
        if (profilePageUrl == null) {
            log.warn("No URL found for reset password");
            profilePageUrl = "";
        }
        return PathUtils.addTrailingSlash(profilePageUrl) + "#/profile";
    }
}
