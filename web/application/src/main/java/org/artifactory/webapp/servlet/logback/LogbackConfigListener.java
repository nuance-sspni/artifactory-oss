...
import static org.jfrog.common.logging.logback.servlet.LoggerConfigInfo.getContextId;
...
public class LogbackConfigListener extends LogbackConfigListenerBase {
    ...
    protected LoggerConfigInfo createLoggerConfigInfo(ServletContext servletContext) {
        return new ArtifactoryLoggerConfigInfo(getContextId(servletContext), home);
    }

    @Override
    protected boolean isDevOrTest() {
        return isTrue(ConstantValues.dev) || isTrue(ConstantValues.devHa) || isTrue(ConstantValues.test);
    }

    private boolean isTrue(@Nonnull ConstantValues value) {
        return "true".equals(System.getProperty(value.getPropertyName()));
    }
}
