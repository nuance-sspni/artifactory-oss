...
public class SetXrayAlertIgnoredService implements RestService {

    private static final Logger log = LoggerFactory.getLogger(SetXrayAlertIgnoredService.class);
    ...
    public void execute(ArtifactoryRestRequest request, RestResponse response) {
        ...
        boolean ignore = Boolean.valueOf(artifact.getParam());
        log.debug("Setting xray.alert.ignored = {} on path {}", ignore, repoPath.toPath());
        addonsManager.addonByType(XrayAddon.class).setAlertIgnored(ignore, repoPath);
        if (ignore) {
            response.info("Alert will be ignored.");
        } else {
            response.info("Ignore alert canceled.");
        }
    }
}
