...
public class RepoConfigModelBuilder {
    ...
    private RemoteNetworkRepositoryConfigModel createNetworkConfig(HttpRepoDescriptor descriptor) {
        ...
        List<String> aliases = webstartAddon.getKeyPairNames();
        if (aliases != null && CollectionUtils.notNullOrEmpty(aliases)) {
            List<String> viewableAliases = aliases.stream()
                    .map(item -> item.substring(ArtifactWebstartAddon.SSL_CERT_ALIAS_PREFIX.length()))
                    .collect(Collectors.toList());
            networkModel.setInstalledCertificatesList(viewableAliases);
        }
        return networkModel;
    }
