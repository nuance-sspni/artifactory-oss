...
public class ArtifactMultiDeployService implements RestService {
    ...
    private void deploy(String savedUniqueFileName, String actualFileName, RestResponse artifactoryResponse,
        ...
        // if local repo not found, try to resolve from virtual default deployment repository (if available)
        if (localRepoDescriptor == null) {
            ...
            if (virtualRepoDescriptor != null) {
                localRepoDescriptor = virtualRepoDescriptor.getDefaultDeploymentRepo();
                RepoPath repoPath = RepoPathFactory.create(repoKey, path);
                if (!authorizationService.canDeploy(repoPath)) {
                    artifactoryResponse.responseCode(HttpServletResponse.SC_FORBIDDEN).buildResponse();
                    log.error("user " + authorizationService.currentUsername() + " has no deploy permissions on " +
                            repoPath.getPath());
                    return;
                }
            }
