                ...
                for (let i = 0; i < this.setMeUpScope.snippets[repoType]['deploy'].length; i++) {
                    ...
                    if (tpl) {
                        tpl = tpl.replace(/\$1/g, repoData.text).replace(/\$2/g, this.setMeUpScope.baseUrl).replace(
                                /\$3/g,
                                this.setMeUpScope.serverId).replace(/\$4/g, this.setMeUpScope.host).replace(/\$5/g, this.setMeUpScope.cleanHost);
                    ...
                    if (tpl && repoData) {
                        tpl = tpl.replace(/\$1/g, repoData.text).replace(/\$2/g, this.setMeUpScope.baseUrl).replace(
                                ...
                                (repoType === 'cocoapods' ? this.setMeUpScope.host.split(':')[0] :
                                        this.setMeUpScope.host)).replace(/\$5/g, this.setMeUpScope.cleanHost);
                        tpl = this._fixTPL(tpl);
                    }
        ...
        if (injectUserData) {
            if (this.injectionData.userName && this.injectionData.password) {
                stringified = stringified.split(
                        ...
                        btoa(this.injectionData.userName.toLowerCase() + ':' + this.injectionData.password));
            }
