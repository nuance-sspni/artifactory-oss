...
public class AqlJsonStreamer extends AqlRestResult implements Cloneable {
    ...
    public AqlJsonStreamer(AqlLazyResult lazyResult, boolean skipResultWrapper) {
    ...
    private Row actOnRow(Row row) {
        try {
            ...
            String err = "Failed to execute action " + action.getName() /* + "on AQL result row {}" + */;
            log.warn(err + " : {}", /*row.itemId*/ e.getMessage());
            log.debug(err, e);
        }
