...
public class FilesTest {
    ...
    public void cleanupEmptyDirectoriesDirWithEmptyNestedDirectoriesAndFilter() {
        File nested1 = createNestedDirectory("com/test");
        File nested2 = createNestedDirectory("org/apache");
        createNestedDirectory("org/apache/empty");

        Files.cleanupEmptyDirectories(baseTestDir, file -> !file.equals(nested1));

        assertTrue(baseTestDir.exists(), "Method should not delete base directory");
        assertFalse(nested2.exists(), "Nested empty directory wasn't deleted");
        assertTrue(nested1.exists(), "Nested empty directory which was supposed to be preserved was deleted");
        File[] files = baseTestDir.listFiles();
        Assert.assertEquals(files.length, 1, "Expected directory with 1 nested directory but received: " + Arrays.asList(files));
    }
